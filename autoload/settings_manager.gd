...
func load_and_apply_settings() -> void:
	...
	# Apply auto-advance
	var auto_advance = config.get_value("text", "auto_advance", false)
	var auto_advance_delay = config.get_value("text", "auto_advance_delay", 1.5)
	ProjectSettings.set_setting(SETTING_AUTOADVANCE_ENABLED, auto_advance)
	ProjectSettings.set_setting(SETTING_AUTOADVANCE_FIXED_DELAY, auto_advance_delay)
